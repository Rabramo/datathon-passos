...
def build_temporal_pair(
    ...
    *,
    ...
    if id_col not in df_t1.columns:
        raise ValueError(f"ID column {id_col} not found in year {year_t1}.")
    ...
    # IMPORTANT: alinhar df_t1 na mesma ordem do df_t
    ...
    # proteção anti-leakage: se existir IAN em t, não usar como feature
    if ian_col in X.columns:
        X = X.drop(columns=[ian_col])
